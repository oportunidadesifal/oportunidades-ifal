...
$app->get('/evento/novo', function (Request $request, Response $response) {
    ...
    return $this->view->render($response, 'eventForm.twig', [
    'user' => $requestUser
    ]);
})->setName('eventForm')->add($sessionAuth);

$app->post('/evento/store', function (Request $request, Response $response, $args) {
    $requestUser = $request->getAttribute('user');
    ...
    $opportunity = new \Oportunista\entities\opportunities\Event($requestBody, 'Event', $requestUserId);
    $opportunityDao = new \Oportunista\entities\opportunities\EventDao($opportunity);
    ...
    return $response = $response->withRedirect($this->router->pathFor('addOpportunity'));
})->setName('eventSave')->add($sessionAuth);